...
// src/components/productsAdmin/ProductAdmin.jsx
// ProductAdmin.jsx
import React, { useState, useEffect } from "react";
import { useNavigate, useLocation } from "react-router-dom";
import axios from "axios";
import AdminProducts from "../adminPage/AdminProducts"; // Assuming AdminProducts is a child component
import styles from "../adminPage/adminPage.module.css";

function ProductAdmin() {
  const navigate = useNavigate();
  const location = useLocation();
  const { selectedCategory } = location.state || null;
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get("http://localhost:8801/products") // Your updated backend endpoint returning all products
      .then((res) => {
  ...
  // Filter products by selected category if available
  const filteredProducts = selectedCategory
    ? allProducts.filter(
        (prod) =>
          // Make sure types match (string or number)
          String(prod.category_id) === String(selectedCategory.category_id)
      )
    : allProducts;
    ...
    <div className={styles.mainContent}>
      ...
      {!loading && (
        <AdminProducts
          ...
          onRefresh={() => {
            // Refresh all products on demand (for example, after add/edit/delete)
            setLoading(true);
            axios
              .get("http://localhost:8801/products/adminView")
              .then((res) => {
              ...
              .catch(() => setError("Failed to reload products"))
              .finally(() => setLoading(false));
          }}
          onShowMessage={({ text, type }) => {
            // You can implement notification here, or just alert for now
